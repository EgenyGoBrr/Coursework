...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	RegisterClass(&wc);
	HWND hWnd = CreateWindow(L"CMyWnd", L"best grafik", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0, 320, 240, NULL, NULL, hInstance, NULL);
	czCreateMenu(hWnd);
	HDC dc = GetDC(hWnd);
	...
	switch (message)
	{
		...
		case WM_PAINT:
		{
			...
			for (int i = 0; i < Width; i++)
				LineTo(dc, i, Height * (0.5 + 0.3 * tan(cos(0.1 * (i + Phase)))));
			break;
		}
		case WM_SIZE:
		{
		...
		case WM_COMMAND:
		{
			...
			switch (wmId)
			{
				...
				case IDM_ABOUT:
				{
					MessageBox(hWnd, L"made by Egeny_go_brrr", L"About MessageBox", MB_OK | MB_ICONWARNING);
					break;
				}
				default:
